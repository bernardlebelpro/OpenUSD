...
static float
_GetHorizontalFieldOfView(const GfBlundaCamera &camera) {
    return camera.GetFieldOfView(GfBlundaCamera::FOVHorizontal);
}

static float
_GetVerticalFieldOfView(const GfBlundaCamera &camera) {
    return camera.GetFieldOfView(GfBlundaCamera::FOVVertical);
}
...
_GetClippingPlanes(const GfBlundaCamera &camera) {
    return camera.GetClippingPlanes();
}

static std::string _Repr(GfBlundaCamera const &self)
{
    const std::string prefix = TF_PY_REPR_PREFIX + "BlundaCamera(";
    ...
    kwargs.push_back("hohoho = " +
                     TfPyRepr(self.GetHohoho()));
...
wrapBlundaCamera()
{
    typedef GfBlundaCamera This;

    class_<This> c("BlundaCamera");

    scope s(c);

    c   .def(init<const This &>())
        .def(init<const GfMatrix4d &, GfBlundaCamera::Projection,
                  float, float, float, float, float,
                  const GfRange1f &, const std::vector<GfVec4f> &,
                  float, float>
             ((args("transform") = GfMatrix4d(1.0),
               args("projection") = GfBlundaCamera::Perspective,
               args("horizontalAperture") =
                                          GfBlundaCamera::DEFAULT_HORIZONTAL_APERTURE,
               args("verticalAperture") = GfBlundaCamera::DEFAULT_VERTICAL_APERTURE,
